...
For AI-only matching, each event batch receives the complete compact playlist candidate list when it fits within `AI_MATCH_CANDIDATE_LIMIT`. With 607 artists, a limit of 1000 avoids the old event-batch × candidate-batch request explosion. Timed-out requests can split into smaller batches, but all requests—including repair and split calls—share `AI_MATCH_MAX_CALLS` and `AI_MATCH_MAX_ELAPSED_SECONDS`. If any required batch fails, the job fails and does not publish a partial result.
